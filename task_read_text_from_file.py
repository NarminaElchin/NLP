...
# Speech function
def speak_text():
    text = text_entry.get()  # Get the input text
...
rate_slider = tk.Scale(root, from_=50, to_=300, orient="horizontal",
                      label="Voice Speed", command=change_rate)
...
volume_slider = tk.Scale(root, from_=0.0, to_=1.0, orient="horizontal",
                        resolution=0.1, label="Volume", command=change_volume)
volume_slider.set(1.0)  # Initial volume
